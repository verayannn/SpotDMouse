...
plt.tight_layout()
plt.savefig('/workspace/all_joints_centering.png', dpi=150)
print("\nSaved all joints centering analysis to: /workspace/all_joints_centering.png")

# Analyze shift patterns
print("\n=== SHIFT PATTERN ANALYSIS ===")
print("\nJoints requiring UPWARD shift (IL too low):")
for joint_idx, shift in joint_shifts.items():
    if shift > shift_threshold:
        print(f"  {joint_names[joint_idx]}: +{shift:.1f}")

print("\nJoints requiring DOWNWARD shift (IL too high):")
for joint_idx, shift in joint_shifts.items():
    if shift < -shift_threshold:
        print(f"  {joint_names[joint_idx]}: {shift:.1f}")

print("\nJoints already in range (no shift needed):")
for joint_idx in range(12):
    if joint_idx not in joint_shifts or abs(joint_shifts[joint_idx]) <= shift_threshold:
        print(f"  {joint_names[joint_idx]}")

# Create scaled IL model with precise shifts
print("\n=== CREATING SCALED IL MODEL WITH PRECISE SHIFTS ===")

# Deep copy the IL model
scaled_IL_MODEL = copy.deepcopy(IL_MODEL)

# Get original last layer parameters
original_last_weight = scaled_IL_MODEL['model_state_dict']['actor.6.weight'].clone()
original_last_bias = scaled_IL_MODEL['model_state_dict']['actor.6.bias'].clone()

# Apply per-joint scaling and shifts
scaling_tensor = torch.tensor(scaling_factors, dtype=torch.float32, device=device)
shift_tensor = torch.tensor([joint_shifts.get(i, 0.0) for i in range(12)], dtype=torch.float32, device=device)

# Scale each output dimension independently
scaled_weight = original_last_weight.clone()
for i in range(12):
    scaled_weight[i, :] = original_last_weight[i, :] * scaling_tensor[i]

scaled_IL_MODEL['model_state_dict']['actor.6.weight'] = scaled_weight
scaled_IL_MODEL['model_state_dict']['actor.6.bias'] = original_last_bias * scaling_tensor + shift_tensor
...
torch.save(scaled_IL_MODEL, output_path)
print(f"\nSaved scaled model to: {output_path}")
...
plt.tight_layout()
plt.savefig('/workspace/model_comparison_precise.png', dpi=150)
print("\nSaved comparison plot to: /workspace/model_comparison_precise.png")

# Create a summary visualization
fig_summary, ax_summary = plt.subplots(1, 1, figsize=(12, 8))

joint_indices = np.arange(12)
shifts = [joint_shifts.get(i, 0.0) for i in range(12)]

# Color code by shift direction
colors = ['green' if abs(s) <= shift_threshold else 'red' if s > 0 else 'blue' for s in shifts]

bars = ax_summary.bar(joint_indices, shifts, color=colors, alpha=0.7)

# Add scaling factors as text
for i, (bar, scale) in enumerate(zip(bars, scaling_factors)):
    height = bar.get_height()
    y_pos = height + np.sign(height) * 3 if abs(height) > shift_threshold else 3
    ax_summary.text(bar.get_x() + bar.get_width()/2., y_pos,
                   f'×{scale:.1f}', ha='center', va='bottom' if height >= 0 else 'top',
                   fontsize=9, fontweight='bold')

# Add a horizontal line at zero
ax_summary.axhline(y=0, color='black', linestyle='-', linewidth=1)
ax_summary.axhline(y=shift_threshold, color='gray', linestyle='--', alpha=0.5)
ax_summary.axhline(y=-shift_threshold, color='gray', linestyle='--', alpha=0.5)

# Customize plot
ax_summary.set_xlabel('Joint')
ax_summary.set_ylabel('Shift Amount')
ax_summary.set_title('Joint-Specific Centering Shifts and Scaling Factors', fontsize=14)
ax_summary.set_xticks(joint_indices)
ax_summary.set_xticklabels([name.replace('-', '\n') for name in joint_names], rotation=45, ha='right')

# Add legend
legend_elements = [
    Patch(facecolor='green', alpha=0.7, label='In range (no shift)'),
    Patch(facecolor='red', alpha=0.7, label='Shift up needed'),
    Patch(facecolor='blue', alpha=0.7, label='Shift down needed')
]
...
plt.tight_layout()
plt.savefig('/workspace/joint_shifts_summary.png', dpi=150)
print("\nSaved joint shifts summary to: /workspace/joint_shifts_summary.png")

# Print summary statistics
print("\n=== OUTPUT STATISTICS SUMMARY ===")
print(f"{'Joint':<12} {'Scale':>6} {'Shift':>7} {'IL Orig Range':>20} {'IL Scaled Range':>20} {'RL Range':>20}")
print("-" * 90)

for i in range(12):
    scale = scaling_factors[i]
    shift = joint_shifts.get(i, 0.0)
    
    orig_min, orig_max = il_outputs_orig[:, i].min(), il_outputs_orig[:, i].max()
    scaled_min, scaled_max = il_outputs_scaled[:, i].min(), il_outputs_scaled[:, i].max()
    rl_min, rl_max = rl_outputs_compare[:, i].min(), rl_outputs_compare[:, i].max()
    
    print(f"{joint_names[i]:<12} {scale:>6.1f} {shift:>7.1f} [{orig_min:>7.3f}, {orig_max:>7.3f}] [{scaled_min:>7.0f}, {scaled_max:>7.0f}] [{rl_min:>7.0f}, {rl_max:>7.0f}]")

# Validation summary - now including variance matching
print("\n=== VALIDATION: CENTER AND VARIANCE MATCHING ===")
print(f"{'Joint':<12} {'RL μ±σ':>15} {'IL Scaled μ±σ':>15} {'Center Δ':>10} {'Var ratio':>10}")
print("-" * 65)

for i in range(12):
    rl_mean = np.mean(rl_outputs_compare[:, i])
    rl_std = np.std(rl_outputs_compare[:, i])
    il_scaled_mean = np.mean(il_outputs_scaled[:, i])
    il_scaled_std = np.std(il_outputs_scaled[:, i])
    
    center_diff = abs(rl_mean - il_scaled_mean)
    var_ratio = il_scaled_std / rl_std if rl_std > 0 else 0
    
    rl_stats = f"{rl_mean:.1f}±{rl_std:.1f}"
    il_stats = f"{il_scaled_mean:.1f}±{il_scaled_std:.1f}"
    
    center_match = "✓" if center_diff < shift_threshold else "✗"
    var_match = "✓" if 0.8 < var_ratio < 1.2 else "✗"
    
    print(f"{joint_names[i]:<12} {rl_stats:>15} {il_stats:>15} {center_diff:>8.1f}{center_match} {var_ratio:>8.2f}{var_match}")

# Create an additional visualization for range comparison
fig_range, ax_range = plt.subplots(figsize=(14, 8))

x_pos = np.arange(12) * 3
width = 0.8

# Plot ranges as bars
for i in range(12):
    # RL range
    rl_min = np.min(rl_outputs_compare[:, i])
    rl_max = np.max(rl_outputs_compare[:, i])
    rl_center = np.mean(rl_outputs_compare[:, i])
    
    # IL scaled range
    il_min = np.min(il_outputs_scaled[:, i])
    il_max = np.max(il_outputs_scaled[:, i])
    il_center = np.mean(il_outputs_scaled[:, i])
    
    # Plot RL range
    ax_range.bar(x_pos[i], rl_max - rl_min, width, bottom=rl_min, 
                 color='green', alpha=0.5, label='RL' if i == 0 else "")
    ax_range.plot(x_pos[i], rl_center, 'go', markersize=8)
    
    # Plot IL scaled range
    ax_range.bar(x_pos[i] + width, il_max - il_min, width, bottom=il_min, 
                 color='red', alpha=0.5, label='IL Scaled' if i == 0 else "")
    ax_range.plot(x_pos[i] + width, il_center, 'ro', markersize=8)

ax_range.set_xlabel('Joint')
ax_range.set_ylabel('Action Value Range')
ax_range.set_title('Joint Action Ranges: RL vs IL Scaled', fontsize=14)
ax_range.set_xticks(x_pos + width/2)
ax_range.set_xticklabels([name.replace('-', '\n') for name in joint_names], rotation=45, ha='right')
ax_range.legend()
ax_range.grid(True, axis='y', alpha=0.3)
ax_range.axhline(y=0, color='black', linestyle='-', linewidth=1)

plt.tight_layout()
plt.savefig('/workspace/joint_range_comparison.png', dpi=150)
print("\nSaved range comparison to: /workspace/joint_range_comparison.png")

print("\nTo view the visualizations:")
print("$BROWSER /workspace/all_joints_centering.png")
print("$BROWSER /workspace/model_comparison_precise.png")
print("$BROWSER /workspace/joint_shifts_summary.png")
print("$BROWSER /workspace/joint_range_comparison.png")

print("\n=== SUMMARY ===")
print(f"Original IL model: {IL_MLP_FILE}")
print(f"Scaled IL model saved to: {output_path}")
print("\nThe scaled model applies joint-specific transformations:")
print(f"  - Scaling factors range from {min(scaling_factors):.1f}x to {max(scaling_factors):.1f}x")
print(f"  - {sum(1 for s in joint_shifts.values() if abs(s) > shift_threshold)} joints needed centering shifts")
print(f"  - {12 - sum(1 for s in joint_shifts.values() if abs(s) > shift_threshold)} joints were already in range")