...
class FixedMappingControllerV4:
    """
    Controller V4
    MAPPING: Derived strictly from user's empirical Sim vs Real test data.
    TUNING:  Rate limits opened up to allow policy execution.
    """
    
    def __init__(self, policy_path="/home/ubuntu/mp2_mlp/policy_joyboy.pt"):
        print("=" * 70)
        print("FIXED MAPPING RL CONTROLLER v4 (Empirical)")
        print("=" * 70)
        ...
        # ==================== JOINT DIRECTION MAPPING ====================
        # DERIVED FROM YOUR TEST DATA:
        # If Sim/Real actions are OPPOSITE -> -1.0
        # If Sim/Real actions are SAME     -> +1.0
        self.joint_direction = np.array([
            -1.0, -1.0, -1.0,   # LF: [Flip, Flip, Flip]
            -1.0, +1.0, +1.0,   # RF: [Flip, Keep, Keep]
            +1.0, -1.0, -1.0,   # LB: [Keep, Flip, Flip]
            +1.0, +1.0, +1.0,   # RB: [Keep, Keep, Keep]
        ])
        
        self.effort_direction = self.joint_direction.copy()
        ...
        # ==================== TUNING (CRITICAL FIXES) ====================
        self.ACTION_SCALE = 0.40          # Sufficient authority
        self.action_smoothing = 0.2       # Low latency
        self.action_rate_limit = 2.0      # OPEN: 0.15 was choking the gait
        
        self.vel_filter_alpha = 0.2
        self.vel_histories = [deque(maxlen=5) for _ in range(12)]
        
        # Base Velocity Estimate (Command-based is safest for now)
        self.estimated_lin_vel = np.zeros(3)
    ...
    def _print_config(self):
        print(f"\n[CONFIG]")
        print(f"  Mapping Source:    Empirical Test Data")
        print(f"  ACTION_SCALE:      {self.ACTION_SCALE}")
        print(f"  action_rate_limit: {self.action_rate_limit}")
    ...
    def _calibrate_sensors(self, samples=50):
        ...
        if gyro_samples: self.gyro_offset = np.mean(gyro_samples, axis=0)
        if accel_samples: self.accel_offset = np.mean(accel_samples, axis=0)
        if effort_samples: self.effort_offset = np.mean(effort_samples, axis=0)
    ...
    def _real_to_sim_positions(self, angles_real):
        deviation_real = angles_real - self.real_default_positions
        deviation_sim = deviation_real * self.joint_direction
        return self.sim_default_positions + deviation_sim

    def _sim_to_real_positions(self, angles_sim):
        deviation_sim = angles_sim - self.sim_default_positions
        deviation_real = deviation_sim * self.joint_direction
        return self.real_default_positions + deviation_real
    
    def _read_joint_positions_sim_frame(self):
        angles_real = self._read_joint_positions_raw()
        if angles_real is None: return self.prev_joint_angles_sim.copy()
        return self._real_to_sim_positions(angles_real)
    
    def _read_joint_efforts(self):
        raw = self.esp32.servos_get_load()
        if raw is None: return np.zeros(12)
        raw = np.array(raw, dtype=float)
        effort_isaac = np.zeros(12)
        offset_isaac = np.zeros(12)
        for hw_col, isaac_leg in self.hw_to_isaac_leg.items():
            for axis in range(3):
                ...
                effort_isaac[isaac_leg * 3 + axis] = raw[servo_id - 1]
                offset_isaac[isaac_leg * 3 + axis] = self.effort_offset[servo_id - 1]
        return ((effort_isaac - offset_isaac) / self.effort_scale) * self.effort_direction

    # ==================== VELOCITY ====================
    def _compute_smoothed_velocity(self, current_angles, dt):
        if dt < 0.001: return self.prev_joint_vel.copy()
        raw_vel = (current_angles - self.prev_joint_angles_sim) / dt
        raw_vel = np.clip(raw_vel, -10.0, 10.0)
        smoothed_vel = np.zeros(12)
        for i in range(12):
            self.vel_histories[i].append(raw_vel[i])
            smoothed_vel[i] = np.median(list(self.vel_histories[i])) if len(self.vel_histories[i]) >= 3 else raw_vel[i]
        return self.vel_filter_alpha * smoothed_vel + (1 - self.vel_filter_alpha) * self.prev_joint_vel

    def _estimate_base_lin_vel(self, accel_body, dt):
        # Stable command-based estimate
        return np.array([self.velocity_command[0] * 0.7, self.velocity_command[1] * 0.7, 0.0])
    ...
    def get_observation(self):
        ...
        accel_norm = np.linalg.norm(accel)
        projected_gravity = accel / accel_norm if accel_norm > 0.1 else np.array([0.0, 0.0, -1.0])
        ...
        return np.concatenate([
            base_lin_vel, base_ang_vel, projected_gravity, self.velocity_command,
            joint_pos_rel, joint_vel, joint_effort, self.prev_actions
        ]).astype(np.float32)
    ...
    def control_step(self):
        ...
        # Startup fade
        fade = min(self.startup_steps / self.startup_duration, 1.0)
        if self.startup_steps < self.startup_duration: self.startup_steps += 1
        faded_actions = clipped_actions * fade
        
        # Smoothing & Rate Limit
        smoothed = self.action_smoothing * faded_actions + (1 - self.action_smoothing) * self.prev_smoothed_actions
        delta = np.clip(smoothed - self.prev_smoothed_actions, -self.action_rate_limit, self.action_rate_limit)
        final_actions = self.prev_smoothed_actions + delta
        ...
        # Convert to Hardware
        angles_real = self._sim_to_real_positions(target_sim)
        matrix = np.zeros((3, 4))
        matrix[:, 1] = angles_real[0:3]   # LF
        matrix[:, 0] = angles_real[3:6]   # RF
        matrix[:, 3] = angles_real[6:9]   # LB
        ...
        self.debug_counter += 1
        if self.debug_counter % 50 == 0:
            print(f"Step {self.debug_counter}: pos=[{obs[12:24].min():.2f},{obs[12:24].max():.2f}] "
                  f"act=[{final_actions.min():.2f},{final_actions.max():.2f}]")
    ...
    def control_loop(self):
        ...
        while not self.shutdown:
            loop_start = time.time()
            if self.control_active:
                try: self.control_step()
                except Exception as e: print(e); self.control_active = False
            elapsed = time.time() - loop_start
            if elapsed < dt_target: time.sleep(dt_target - elapsed)
...
if __name__ == "__main__":
    controller = FixedMappingControllerV4()
    t = threading.Thread(target=controller.control_loop, daemon=True)
    t.start()
    
    speed = 0.15
    print("Controls: w/s (speed), space (stop), x (exit)")
    
    try:
        while True:
            c = input("> ").strip().lower()
            if c == 'w': controller.set_velocity_command(speed, 0, 0)
            elif c == 's': controller.set_velocity_command(-speed, 0, 0)
            elif c == ' ': controller.set_velocity_command(0, 0, 0)
            elif c == 'x': break
            elif c == 'debug': controller._print_config()
